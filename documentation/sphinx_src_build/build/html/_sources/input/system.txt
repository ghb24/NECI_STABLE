...
* Read in data produced by a molecular computational chemistry package:
    ...
    **STARBINREAD**
        Like **STARSTOREREAD**, but FCIDUMP is a binary file.
    **DFREAD**
...
**ORDER**
  ...
  **STARBINREAD**, **STARSTOREREAD**, **BINREAD**, or **DFREAD** then
  perform a quick HF calculation in the space of the orbitals.
  The orbitals are then reordered according to the HF energies,
  rather than using the orbital energies read in.  If **CPMD** is followed
  by **ORDER**, then the CPMD orbitals are ordered, not according to their
  Kohn--Sham eigenvalues, but instead according to their one-electron energies
  (i.e. with no exchange or correlation).  **ORDER** is not valid for any
  other system type.
