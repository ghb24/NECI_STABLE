...
!.. Based on SortI, this will sort a list of INTEGERS, and order the next integer(KIND=8) array and further integer array according to the first.
      SUBROUTINE SORTIILongI(N,RA,RB,RC)
      IMPLICIT NONE
      INTEGER N,I,L,IR,J
      INTEGER(KIND=SELECTED_INT_KIND(18)) :: RB(N)
      INTEGER(KIND=SELECTED_INT_KIND(18)) :: RRB
      INTEGER :: RC(N)
      INTEGER :: RRC
      INTEGER RA(N)
      INTEGER RRA
...
!.. Based on SortI, this will sort a list of INTEGERS, and order the next integer(KIND=8) array and logical array according to the first.
      SUBROUTINE SORTIILongL(N,RA,RB,RC)
      IMPLICIT NONE
      INTEGER N,I,L,IR,J
      INTEGER(KIND=SELECTED_INT_KIND(18)) :: RB(N)
      INTEGER(KIND=SELECTED_INT_KIND(18)) :: RRB
...
!Based on SortI, this will sort a list of INTEGER(KIND=SELECTED_INT_KIND(18)) not according to its value, but according to the value of abs(mod(RA,NProc))
!It will then order the next three integer arrays and logical array according to the first.
!This is used for parallel annihilation when we need to take an extra integer information with us
      SUBROUTINE SORTMod4I1LLong(N,RA,RB,RC,RD,RE,NProc)
      IMPLICIT NONE
      INTEGER N,I,L,IR,J,NProc
      INTEGER(KIND=SELECTED_INT_KIND(18)) :: RA(N)
      INTEGER(KIND=SELECTED_INT_KIND(18)) :: RRA,nProcLong
