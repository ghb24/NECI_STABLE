...
def read_cols (f, last_iter=None, last_im_time=None):
	...
	for line in f:
		...
		if last_iter and bits[0] > last_iter:
			break
		if last_im_time and bits[18] > last_im_time:
			break
	...
	def plot_data (self):
		...
		for i, fl in enumerate(self.in_files):
			...
			with open(fl.fn, 'r') as f:
				...
				it, sft, none1, growth, wlk, ann, died, born, avProj, avSft, \
						proj, atRef, none3, accept, none4, itime, none5, \
						none6, im_time = cols[0:19]
				proj_tot = cols[22]

				# Some data may not be in certain files.
				try:
					S2, S2_init = cols[27:29]
				except:
					if self.S.plot:
						if self.verbose:
							print 'Missing spin data, not plotting'
						S2 = zeros((len(it)))
						S2_init = zeros((len(it)))
				try:
					exp_E = cols[34]
				except:
					if self.E.plot_exp_average:
						print 'Missing exp average data, not plotting'
						exp_E = zeros((len(it)))
				...
				if fl.ofile is not None and self.E.total_energies:
					...
					sft = apply_ref_Es(sft, it, ref_E)
					if self.E.plot_exp_average:
						exp_E = apply_ref_Es(exp_E, it, ref_E)
					if self.E.plot_averages:
						avProj = apply_ref_Es(avProj, it, ref_E)
						avSft = apply_ref_Es(avSft, it, ref_E)
				...
				if self.E.plot:
					...
					ax.plot (x, sft, col()+fmt, label=' '.join([leg_pre,'Inst. Shift']))
					if self.E.total_energies:
						ax.plot (x, proj_tot, col()+fmt, label=' '.join([leg_pre,'Proj. E']))
					else:
						ax.plot (x, proj, col()+fmt, label=' '.join([leg_pre,'Proj. E']))
					if self.fit_walkers_E:
						fit, efit, err = stretched_exp_fit(wlk, proj)
						ax.plot (x, fit, col(), label=' '.join([leg_pre,'FIT']))
						ax.axhline(efit, color=col(), label=' '.join([leg_pre,"E-fit"]))
					if self.E.plot_averages:
						ax.plot (x, avProj, col()+fmt, label=' '.join([leg_pre,'Av. Proj. E']))
						ax.plot (x, avSft, col()+fmt, label=' '.join([leg_pre,'Av. Shift']))
					if self.E.plot_exp_average:
						ax.plot (x, exp_E, col()+fmt, label=' '.join([leg_pre,'Exp. Average']))
				...
				if self.W.plot:
					if not self.share_walkers_E:
						col.reset()
					ax = self.W.ax
					ax.plot (x, wlk, col(), label=' '.join([leg_pre,'No. Walkers']))
					ax.plot (x, atRef, col(), label=' '.join([leg_pre,'No. at Ref']))

					if self.W.plot_growth_cpts:
						ax.plot (x, born, col(), label=' '.join([leg_pre,'No. Born']))
						ax.plot (x, died, col(), label=' '.join([leg_pre,'No. Died']))
						ax.plot (x, ann, col(), label=' '.join([leg_pre,'No. Annihil']))
				...
				if self.S.plot:
					...
					ax = self.S.ax
					if self.S.log_scale:
						X, S = zip(*[(i, s) for i, s in zip(x, S2) if s > 0])
					else:
						X, S = x, S2
					ax.plot (X, S, col(), label=' '.join([leg_pre,'Inst. $S^2$']))
					
					if self.S.log_scale:
						X, S_init = zip(*[(i, s) for i, s in zip(x, S2_init) if s > 0])
					else:
						X, S_init = x, S2_init
				...
				if self.T.plot:
					...
					col.reset()
					if self.T.plot_tau:
						timestep = get_timestep(it, im_time, tau_changes)
						ax.plot (x, timestep, col(), label=' '.join([leg_pre,'Timestep (a.u.)']))
					if self.T.plot_iter_time:
						ax.plot (x, itime, col(), label=' '.join([leg_pre,'Iteration Time']))

				# Plot survival/acceptance related details
				if self.U.plot:
					ax = self.U.ax
					col.reset()
					ax.plot (x, accept, col(), label=' '.join([leg_pre,'Acceptance rate']))

				if self.G.plot:
					print "Plotting G"

					# Get the growth rate for the reference
					grow_ref = [1.0] + [r2 / r1 for (r1, r2) in zip(atRef, atRef[1:])]
					growth_ratio = array(growth) / array(grow_ref)
