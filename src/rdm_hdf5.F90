...
!         call h5gclose_f(onerdm_grp_id, err)
    ...
    subroutine write_2rdm_hdf5(parent, rdm, rdm_trace, iroot)
        ...
        real(dp), allocatable :: values(:)
        integer(hsize_t) :: dimsf(1), cnt(1), offset(1)
        ...
        cnt = 1
        call h5screate_simple_f(rank, cnt, memspace, err)
        ! create a dummy memspace for null-writing
        call h5screate_simple_f(rank, cnt, none_memspace, err)
        call h5sselect_none_f(none_memspace, err)
        ...
        do p = 1, int(max_length_RDM)
            ...
            if (size(values) < p) then
                ...
                call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, 0.0_dp, cnt, err, &
                                file_space_id=filespace, mem_space_id=none_memspace, &
                                xfer_prp=plist_id)
            else
                call h5sselect_hyperslab_f(filespace, H5S_SELECT_SET_F, offset, &
                                           cnt, err)
                call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, values(p), cnt, err, &
