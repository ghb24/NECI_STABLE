...
!which can be read in in a single batch.
    SUBROUTINE ReadFromPopsfile(EndPopsList,ReadBatch,CurrWalkers64,CurrParts,CurrHF,Dets,DetsLen,PopNifSgn)
        ...
        integer :: iunit,i,j,ierr,PopsInitialSlots(0:nNodes-1), iunit_3
...
r_loop: do while(.not.tReadAllPops)
            ...
            if (iProcIndex == root .or. (bNodeRoot .and. tSplitPops)) then
                ...
                do j = 0, nNodes - 1
                    ...
                    if(tReadRDMAvPop) then
                        ...
                        disps2(j+1) = &
                            int((PopsInitialSlots(j) - 1) * (1+2*lenof_sign), MPIArg)
                    endif
                enddo
            ...
            if (bNodeRoot) then
                ...
                if(tReadRDMAvPop) then
                    ...
                    call MPIScatterV (BatchCurrentH(:,1:MaxSendIndex), sendcounts2, &
                                      disps2, & 
...
!            WRITE(6,*) i,Dets(:,i)
    ...
    function read_popsfile_det (iunit, Det, BinPops, WalkerTemp, nI, PopNifSgn, CurrentHEntry, iunit_3) &
                               result(tEOF)
...
r_loop: do while(.not.tStoreDet)
            ...
            if (.not. tPopsMapping) then
                ...
                if (tReadRDMAvPop) read(iunit_3, iostat=stat) CurrentHEntry(1:1+2*lenof_sign)
