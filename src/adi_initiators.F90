module adi_initiators
  use FciMCData, only: ilutRefAdi, nRefs
  ...
  use CalcData, only: tAllDoubsInitiators, tAllSingsInitiators

contains

  !------------------------------------------------------------------------------------------!

  function check_static_init(ilut, sgn, run) result(staticInit)
    use bit_rep_data, only: flag_adi_checked, flag_static_init
    use FciMCData, only: tReferenceChanged
    implicit none
    ...
    if(tAllDoubsInitiators .or. tAllSingsInitiators) then
       if(test_flag(ilut, flag_adi_checked)) then
          ...
          ! only for this run
          if(check_sign_changed(ilut, sgn, run)) call set_adi_flags_run(ilut, sgn, run)
       else
          ! else, evaluate all adi flags and then set flag_adi_checked
          call set_adi_flags(ilut, sgn)
       endif
  ...
  subroutine set_adi_flags(ilut, sgn)
    ...
    do ir = 1, inum_runs
       call set_adi_flags_run(ilut, sgn, ir)
    end do
  ...
  subroutine set_adi_flags_run(ilut, sgn, ir)
    ...
    staticInit = adi_criterium(ilut, sgn, ir)
    if(staticInit) then
       ! if so, set the flag
  ...
  function adi_criterium(ilut, sgn, run) result(staticInit)
    ...
    do i = 1, nRefs
       ...
       if(exLevel < 3) then
          ...
          if(unset_incoherent_initiator(exLevel, ilut, sgn, i, staticInit, run)) &
  ...
  function unset_incoherent_initiator(exLevel, ilut, sgn, iRef, &
    ...
    if(tCoherentDoubles .and. (exLevel > 0)) then
       if(.not. check_sign_coherence(ilut,sgn,iRef,run)) then
  ...
  subroutine set_double_initiator(exLevel, staticInit)
    use FciMCData, only:  nCoherentDoubles
    implicit none
  ...
  subroutine set_single_initiator(exLevel, staticInit)
    use FciMCData, only: nCoherentSingles
    implicit none
    integer, intent(in) :: exLevel
