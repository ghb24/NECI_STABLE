...
contains
    ...
    subroutine pick_orbitals_pure_uniform_singles(ilut, nI, csf_i, excitInfo, pgen)
        ...
        call pchb_sampler%pick_orbitals_pure_uniform_singles(ilut, nI, csf_i, excitInfo, pgen)
    end subroutine

    subroutine pick_orbitals_double_pchb(ilut, nI, csf_i, excitInfo, pgen)
        integer(n_int), intent(in) :: ilut(0:GugaBits%len_tot)
        integer, intent(in) :: nI(nel)
        type(CSF_Info_t), intent(in) :: csf_i
        type(ExcitationInformation_t), intent(out) :: excitInfo
        real(dp), intent(out) :: pgen
        call pchb_sampler%pick_orbitals_double_pchb(ilut, nI, csf_i, excitInfo, pgen)
    end subroutine

    ! I need the pgen-recalculation routines for exchange type excitations
    ! also for the PCHB excit-gen
    pure subroutine calc_orbital_pgen_contr_pchb(csf_i, occ_orbs, cpt_a, cpt_b)
        type(CSF_Info_t), intent(in) :: csf_i
        integer, intent(in) :: occ_orbs(2)
        real(dp), intent(out) :: cpt_a, cpt_b
        call pchb_sampler%calc_orbital_pgen_contr_pchb(csf_i, occ_orbs, cpt_a, cpt_b)
    end subroutine calc_orbital_pgen_contr_pchb


    ! i think it would be better if i 'just' reimplement:
    pure subroutine calc_orbital_pgen_contr_start_pchb(csf_i, occ_orbs, a, orb_pgen)
        type(CSF_Info_t), intent(in) :: csf_i
        integer, intent(in) :: occ_orbs(2), a
        real(dp), intent(out) :: orb_pgen
        call pchb_sampler%calc_orbital_pgen_contr_start_pchb(csf_i, occ_orbs, a, orb_pgen)
    end subroutine
    ...
    function calc_pgen_guga_pchb(ilutI, csf_i, ilutJ, excitInfo) result(pgen)
        ...
        real(dp) :: pgen
        pgen = pchb_sampler%calc_pgen_guga_pchb(ilutI, csf_i, ilutJ, excitInfo)
    end function
