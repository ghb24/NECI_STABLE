...
!        write(6,*) 'dets', currentdets(noffsgn, 1:totwalkers)
    ...
    subroutine generate_fval_histogram(hist, histEnergy, histAccRate, enPoints, accRatePoints)
      ...
      real(dp), allocatable, intent(out) :: histEnergy(:), histAccRate(:)
      integer, intent(in) :: enPoints, accRatePoints
      integer :: i, run
      integer :: enInd, arInd
      real(dp) :: minEn, maxEn, enWindow, arWindow

      ! get the energy window size (the acc. rate window size is just 1.0/(accRatePoints+1))
      minEn = det_diagH(1)
      maxEn = det_diagH(1)
      do i = 2, TotWalkers
         if(det_diagH(i) > maxEn) maxEn = det_diagH(i)
         if(det_diagH(i) < minEn) minEn = det_diagH(i)
      end do
      enWindow = maxEn - minEn

      ! set up the histogram axes
      allocate(histAccRate(accRatePoints))
      arWindow = 1.0_dp / real(1+accRatePoints,dp)
      do i = 1, accRatePoints
         histAccRate(i) = (i+1.0_dp/2.0_dp)*arWindow
      end do
      ...
      allocate(hist(accRatePoints,enPoints))
      hist = 0
      do i = 1, TotWalkers
         do run = 1, inum_runs
            enInd = getHistIndex(det_diagH(i), minEn, enWindow)
            arInd = getHistIndex(get_acc_spawns(i,run)/get_tot_spawns(i,run), &
                 0.0_dp, arWindow)
            hist(arInd, enInd) = hist(arInd, enInd) + 1
         end do
      end do

    contains

      function getHistIndex(val, minVal, windowSize) result(ind)
        ! for a given energy, get the position in the histogram
        implicit none
        real(dp), intent(in) :: val, minVal, windowSize
        integer :: ind

        ind = int((val - minVal) / windowSize)
