subroutine HPHFGetOffDiagHElement(nI,nJ,iLutnI,iLutnJ,MatEl)
    ...
    ! Find the HELement between two half-projected hartree-fock determinants
    ! (different ones). NI and nJ have to be uniquely chosen, so that their
    ! spin-coupled determinant will not arise.
    ! TODO: Modularise
    !
    ! In:  nI, nJ         - Determinants to consider
    !      iLutnI, iLutnJ - Bit representations of I,J
    ! Out: MatEl          - The calculated matrix element
    ...
    type(HElement), intent(out) :: MatEl ! TODO: convert to fn return value.
    ...
    if (DetBitEQ(iLutnI, iLutnJ, NIfDBO)) then
        ! Do not allow an 'off-diagonal' matrix element. The problem is that 
        ! the HPHF excitation generator can generate the same HPHF function. 
        ! We do not want to allow spawns here.
        MatEl = HElement(0)
        return
    endif
    ...
    if (TestClosedShellDet(iLutnI)) then
        if (.not. TestClosedShellDet(iLutnJ)) then
            ...
            ! or with a minus if iLutnJ has an odd number of spin orbitals.
            ! OTHERWISE Closed shell -> closed shell. Both the alpha and beta
            ! of the same orbital have been moved to the same new orbital. The
            ! matrix element is the same as before.
            MatEl = MatEl * HELement(sqrt(2.d0))
        endif
    else
        if (TestClosedShellDet(iLutnJ)) then
            ! Open shell -> Closed shell. I am pretty sure that if one of the
            ! determinants is connected, then the other is connected with the
            ! same IC (+matrix element?). Test this later.
            MatEl = MatEl * HElement(sqrt(2.d0))
        else
            ...
            if (ExcitLevel.le.2) then
                ...
                ! symmetric or antisymmetric. This is dependant on the number
                ! of open shell orbitals.
                ...
                if (((mod(OpenOrbsI,2) == 0) .and. (mod(OpenOrbsJ,2) == 0)) &
                    .or. ((mod(OpenOrbsI,2) == 0) .and. (mod(OpenOrbsJ,2) == 1))) then
                    MatEl=MatEl+MatEl2
                else
                    MatEl=MatEl-MatEl2
                endif
...
subroutine HPHFGetDiagHElement(nI, iLutnI, MatEl)
    ...
    ! Find the diagonal HElment for a half-projected hartree-fock determinant.
    ! TODO: Modularise
    !
    ...
    type(HElement), intent(out) :: MatEl ! TODO: make as fn ret value
    ...
    MatEl = sltcnd_excit (nI, nI, 0)
    if (.not. TestClosedShellDet(iLutnI)) then
        ! <i|H|i> = <j|H|j>, so no need to calculate both.
        ! <X|H|X> = 1/2 [ <i|H|i> + <j|H|j> ] + <i|H|j> where i and j are the 
        ! two spin-coupled dets which make up X. In the case of the 
