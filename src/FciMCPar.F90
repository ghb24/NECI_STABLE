        ...
        do i=1, NumDoubEntries+NumQuadEntries
            ...
            if (child(1) .ne. 0) then
                ...
                call new_child_stats (iter_data, iLutCurr, &
                                      nJ, iLutCurr, ic, nullwalkExcitLevel,&
       ...
       if(proje_spatial) then
        ...
        if(tSplitProjEHist.and.ExcitLevel_local == 2) then
            ...
            if (tSplitProjEHistK3) then 
                ...
                ! Sum in energy contribution
                ENumCycHistK3(ProjEBin) = ENumCycHistK3(ProjEBin) + (HOffDiag * ARR_RE_OR_CPLX(wSign)) / dProbFin
            endif
...
!Add LogMemAllocs
        ...
        if (tCISDref) then
            ...
            Allocate(CISDIntFluxPosition(NifD+1,DoubDetsEst))
            ...
            Allocate(CISDOutFluxPosition(NifD+1,QuadDetsEst))
            CISDIntFlux(:,:) = 0.D0
            CISDOutFlux(:,:) = 0.D0
            CISDIntFluxPosition(:,:) = 0
            CISDOutFluxPosition(:,:) = 0
            NumDoubEntries=0
            NumQuadEntries=0
            SortedDoubEntries=0
            SortedQuadEntries=0
...
!                call WriteBitEx(iout,iLutRef,GlobalLargestWalkers(:,i),.false.)
                ...
                call extract_sign(GlobalLargestWalkers(:,i),SignCurr)
                do j=1,lenof_sign
                    write(iout,"(I9)",advance='no') SignCurr(j)
                enddo
                if(lenof_sign.eq.1) then
                    HighSign=real(abs(SignCurr(1)),dp)
                else
    ...
    Subroutine Store_cisd_coeffs(iLut, wSign, j)
    ...
    SUBROUTINE calc_cisd_flux()
        ...
        Allocate(CISDref(NifD+1+lenof_sign,MaxTotWalkers))
        CISDref(:,:) = 0

        call cisd_ref_allocation 
       
        WRITE(6,*) "Max # CISD Dets per core", MaxTotWalkers
        WRITE(6,*) "Processing the CISD wavefunction to calculate internal and external flux"
        ...
        do i=1,MaxTotWalkers
            ...
            if (walkExcitLevel .eq. 0) then
                CISDHFCoeff=SignCurr(1)
            elseif (walkExcitLevel .le. 2) then
                call calc_CISD_proje_contrib(iLutnI, nI, SignCurr, walkExcitLevel)
            endif

            call store_cisd_coeffs(CurrentDets(:,i), SignCurr, i)
            ...
            if (.not. blank_det) then
                posn=binary_search(CISDIntFluxPosition(:,DoubDetsEst-NumDoubEntriesDoubDetsEst), iLutnI(0:NifD))
                if (posn<0) then
                    !WRITE(6,*) "iLutnI not found", iLutnI(:)
                    !WRITE(6,*) "CISDIntFlux", CISDIntFluxPosition(:,:)
                    !New entry
                    NumDoubEntries=NumDoubEntries+1
                    posn=DoubDetsEst-NumDoubEntries+1
                    UnsortedDoubEntries=UnsortedDoubEntries+1
                    !if(CISDIntFlux(1,posn).ne.0) then
                    !    WRITE(6,*) "ERROR1"
                        !WRITE(6,*) "posn", posn
                        WRITE(6,*) "iLutnI", iLutnI
                        WRITE(6,*) "nI", nI
                        WRITE(6,*) "Num", NumDoubEntries
                        WRITE(6,*) CISDIntFlux(:,DoubDetsEst-200:DoubDetsEst)
                    !endif
                  !  WRITE(6,*) "posn", posn
                else
                    posn=posn+DoubDetsEst-NumDoubEntries-1
                    if((CISDIntFlux(1,posn).ne.iLutnI(0))) then
                        WRITE(6,*) "ERROR2"
                        WRITE(6,*) "posn", posn
                        WRITE(6,*) "iLutnI", iLutnI
                        WRITE(6,*) "Num, unsorted, sorted", NumDoubEntries, UnsortedDoubEntries, SortedDoubEntries
                        WRITE(6,*) CISDIntFlux(:,:)
                    endif
                   ! WRITE(6,*) "posn", posn
                endif
                ...
                call sort (CISDIntFluxPosition(:,DoubDetsEst-NumDoubEntries+1:DoubDetsEst))
                SortedDoubEntries=SortedDoubEntries+UnsortedDoubEntries
                UnsortedDoubEntries=0
            endif
        
            !do k=DoubDetsEst-NumDoubEntries, DoubDetsEst
            !    if ((CISDIntFlux(1,k).eq.0).and.(CISDIntFlux(2,k).eq.0)) then
            !        if (k .le. DoubDetsEst-NumDoubEntries) then
            !            cycle
            !        else
            !            WRITE(6,*) "ERROR HERE"
            !            WRITE(6,*) "iLutnI", iLutnI
            !            WRITE(6,*) "posn", posn
            !            WRITE(6,*) "k", k
            !            WRITE(6,*) "CISDIntFlux", CISDIntFlux(:,:)
            !            WRITE(6,*) "DoubDetsest, NumDoubEntries", DoubDetsEst, NumDoubEntries
            !        endif
            !    else
            !        if (k .le. DoubDetsEst-NumDoubEntries) then
            !            WRITE(6,*) "ERROR HERE"
...
! D_i is in the first spot - start from the second.                
                    ...
                    WRITE(6,*) CISDIntFlux(:,DoubDetsEst-200:DoubDetsEst)
                     ...
                     !           endif
                                ...
                                if((CISDIntFlux(1,posn).ne.iLutnJ(0))) then
                                    WRITE(6,*) "ERROR4"
                                    WRITE(6,*) "posn", posn
                                    WRITE(6,*) "iLutnJ", iLutnJ
                                    WRITE(6,*) "Num, unsorted, sorted", NumDoubEntries, UnsortedDoubEntries, SortedDoubEntries
           ...
           ! enddo
                            ...
                            if (posn<0) then
                                ...
                                if (posn.eq.0) then
                                    WRITE(6,*) "CISDOutFlux(:,:)", CISDOutFlux(:,:)
                                endif
                                if(CISDOutFlux(1,posn).ne.0) then
                                    WRITE(6,*) "ERROR5"
                                    WRITE(6,*) "posn", posn
                                    WRITE(6,*) "iLutnJ", iLutnJ
                                    WRITE(6,*) "Num, unsorted, sorted", NumQuadEntries, UnsortedQuadEntries, SortedQuadEntries
                                    WRITE(6,*) CISDOutFlux(:,:)
                                endif
                                !WRITE(6,*) "posn, QuadDetsest, SortedQuadEntries, UnsortedQuadEntries", &
                               !     posn, QuadDetsest, SortedQuadEntries, UnsortedQuadEntries
                                call neci_flush(6)
                            else
                                posn=posn+QuadDetsEst-SortedQuadEntries-1
                                !WRITE(6,*) "posn1, QuadDetsest, SortedQuadEntries, UnsortedQuadEntries", &
                                !    posn, QuadDetsest, SortedQuadEntries, UnsortedQuadEntries
                                call neci_flush(6)
                                if((CISDOutFlux(1,posn).ne.iLutnJ(0))) then
                                    WRITE(6,*) "ERROR6"
                                    WRITE(6,*) "posn", posn
                                    WRITE(6,*) "iLutnJ", iLutnJ
                                    WRITE(6,*) "Num, unsorted, sorted", NumQuadEntries, UnsortedQuadEntries, SortedQuadEntries
            ...
            !do k=QuadDetsEst-NumQuadEntries, QuadDetsEst
            !    if ((CISDOutFlux(1,k).eq.0).and.(CISDOutFlux(2,k).eq.0)) then
            !        if (k .le. QuadDetsEst-NumQuadEntries) then
            !            cycle
            !        else
            !            WRITE(6,*) "ERROR HERE"
            !            WRITE(6,*) "iLutnJ", iLutnJ
            ...
            !           WRITE(6,*) "k", k
            !            WRITE(6,*) "CISDOutFlux", CISDOutFlux(:,:)
            !            WRITE(6,*) "QuadDetsest, NumQUadEntries", QuadDetsEst, NumQuadEntries
            !        endif
            !    else
            !        if (k .le. QuadDetsEst-NumQuadEntries) then
            !            WRITE(6,*) "ERROR HERE"
            ...
            call sort (CISDOutFluxPosition(:,QuadDetsEst-NumQuadEntries+1:QuadDetsEst))
    ...
    !do i=1, DoubDetsEst
    !    WRITE(6,*) "i, CISDIntFlux(:,i)", i, CISDIntFlux(:,i)
    !enddo
    ...
    do i=1,DoubDetsEst
        ...
        if ((CISDIntFlux(1,i).eq. 0).and.(CISDIntFlux(2,i).eq.0) .and. (CISDIntFlux(3,i).eq.0)) then
            ...
            posn1=binary_search(CISDref(:,:), CISDIntFluxPosition(:,i), NifD+1)
            if (posn1 .lt. 0) then
            ...
            !if(posn.gt.7865) then
            !    WRITE(6,*) CISDIntFlux(:,i)
            !    WRITE(6,*) CISDTotFlux(:,posn)
            !endif
    ...
    WRITE(6,*) "Doub posn, NumDoubEntries", posn-1, NumDoubEntries
    ...
    enddo
    WRITE(6,*) "Quad posn, Quad+NumDoubEntries", posn-1, NumDoubEntries+NumQuadEntries

    !WRITE(6,*) "Before sort", CISDTotFlux(:,1:NumDoubEntries+NumQuadEntries)
    ...
    FUNCTION attempt_create_staticflux (CISDRefCoeff,iLutnJ, StaticFlux) result(child)

        integer(kind=n_int), intent(inout) :: iLutnJ(0:niftot) !Determinant being spawned onto
        real(dp), intent(in) :: StaticFlux
        integer, intent(in) :: CISDRefCoeff
        integer, dimension(lenof_sign) :: child
        real(dp) :: rat, r
        integer :: extracreate
        ...
        rat = tau * abs(StaticFlux-DiagSft*CISDRefCoeff)
        ...
        if (rat > r) then
            !Create child
            child(1) = -nint(sign(1.0_dp, StaticFlux-DiagSft*CISDRefCoeff))
            child(1) = child(1) + sign(extraCreate, child(1))
        elseif(extraCreate.ne.0) then
            !Just return if any extra particles created
    ...
    SUBROUTINE calc_CISD_proje_contrib(iLut, nI, wSign, ExcitLevel)
        ...
        CISDProjEContrib = CISDProjEContrib + real(HOffDiag * ARR_RE_OR_CPLX(wSign))
        CISDProjEContribAbs = CISDProjEContribAbs + real(abs(HOffDiag * ARR_RE_OR_CPLX(wSign)))
