...
module gasci_general_pchb
    ...
    use util_mod, only: choose, stop_all, cumsum, binary_search_first_ge, intswap
    ...
    use util_mod, only: fuseIndex, getSpinIndex, near_zero
...
!
    type :: GAS_PCHB_excit_gen_t
        private
        !> The dimension is (3, n_supergroup)
...
contains
    ...
    logical pure function is_allowed(self, exc, supergroup)
        ...
        if (all(src_spaces == tgt_spaces) .and. src_spaces(1) == src_spaces(2)) then
            ...
            excited_supergroup(src_spaces) = supergroup(src_spaces) - 1
            excited_supergroup(tgt_spaces) = supergroup(tgt_spaces) + 1
...
!
    ...
    subroutine finalize(this)
        ...
        integer :: samplerIndex, i_sg

        do i_sg = 1, size(this%pchb_samplers, 2)
            do samplerIndex = 1, 3
