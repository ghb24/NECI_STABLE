...
module global_det_data
    ...
    use FciMCData, only: global_determinant_data
    use constants
    implicit none
...
contains
    ...
    subroutine clean_global_det_data()
        ...
        if (allocated(global_determinant_data)) then
    ...
    subroutine set_det_diagH(j, hel_r)
        ...
        global_determinant_data(pos_hel, j) = hel
    ...
    subroutine set_av_sgn (j, part, av_sgn)
    ...
    function get_av_sgn (j, part) result(av_sgn)
    ...
    subroutine set_iter_occ (j, part, iter_occ)
    ...
    function get_iter_occ (j, part) result(iter_occ)
