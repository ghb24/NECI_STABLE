...
!Then J...
        ...
        else if (EXCITFUNCS(3)) THEN
            IF ((Arr(I, 2) + Arr(J, 2)) > g_VMC_ExcitWeights(1, CUR_VERT)) Weight = 1.0_dp
            IF ((Arr(I, 2) + Arr(J, 2)) <= g_VMC_ExcitWeights(1, CUR_VERT)) THEN
...
!            call halt_timer(proc_timer)
        ...
        IF (.not. EXCITFUNCS(10)) THEN
            IF ((EXCITFUNCS(1)) .and. (.not. near_zero(g_VMC_ExcitWeights(3, CUR_VERT)))) THEN
                ...
                IF (ABS(W2) < 1.0e-2_dp) W2 = 1.0e-2_dp
...
!Step-function weighting using chemical potential cut-off
                IF (Arr(K, 2) > CHEMPOT) THEN
                    W2 = g_VMC_ExcitWeights(2, CUR_VERT)
                ELSE
                    W2 = 1.0_dp
                end if
                IF (Arr(L, 2) > CHEMPOT) THEN
...
!chempotweighting - using a chemical potential cut-off
                ...
                IF ((Arr(K, 2) + Arr(L, 2)) < (CHEMPOT * 2.0_dp)) Weight = Weight
                IF ((Arr(K, 2) + Arr(L, 2)) >= (CHEMPOT * 2.0_dp)) THEN
...
!PolyExcitWeighting
            else if (EXCITFUNCS(2)) THEN
                IF ((Arr(K, 2) + Arr(L, 2)) < g_VMC_ExcitWeights(2, CUR_VERT)) Weight = Weight
                IF ((Arr(K, 2) + Arr(L, 2)) >= g_VMC_ExcitWeights(2, CUR_VERT)) THEN
...
!.. Now find which excittype we correspons to
        K = 1
        DO WHILE (ExcitTypes(1, K) /= 2 .OR. ExcitTypes(2, K) /= iSpn .OR. ExcitTypes(3, K) /= iFrom .OR. ExcitTypes(4, K) /= iTo)
            K = K + 1
        end do
...
!  Now enumerate all the FROMs, and their weightings
        ...
        DO I = 1, nFromPairs
            IF (ews(I)%I == iExcit(1, 1) .AND. ews(I)%J == iExcit(1, 2)) THEN
...
!.. The prob of all possible excitations in this iTo
        DO I = 1, nToPairs
            IF (ews(I)%A == iExcit(2, 1) .AND. ews(I)%B == iExcit(2, 2)) THEN
