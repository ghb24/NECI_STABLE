...
!#endif
    ...
    subroutine generate_space(core_in)
        ...
        if (core_in%tPops) call generate_space_most_populated(core_in%npops, & 
                                    core_in%tApproxSpace, core_in%nApproxSpace, SpawnedParts, space_size, CurrentDets, int(TotWalkers))
...
!                     if (abs(real(HEl,dp)) < 1.e-12_dp) cycle
    ...
    subroutine write_most_pop_core_at_end(target_space_size)
        ...
        call generate_space_most_populated(target_space_size, .false., 0, SpawnedParts, space_size, CurrentDets, int(TotWalkers))
