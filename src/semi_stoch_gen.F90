...
contains
    ...
    subroutine init_semi_stochastic(core_in, tStartedFromCoreGround)
        ...
        use SystemData, only: tAllSymSectors, tReltvy, nOccAlpha, nOccBeta
...
!             loc_source => CurrentDets
    ...
    subroutine generate_fci_core(ilut_list, space_size)
        ...
        if (.not. tSpn) call stop_all(t_r, "To use the FCI-CORE option you must fix the total spin on z-axis, e.g., &
                                              & by using 'SPIN-RESTRICT' option" )
