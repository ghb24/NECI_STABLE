...
contains
    ...
    subroutine generate_space()
        ...
        if (tStartCAS) then
            do i = 1, TotWalkers
                ...
                call set_flag(CurrentDets(:, i), flag_initiator(1))
                call set_flag(CurrentDets(:, i), flag_initiator(2))
            end do
    ...
    subroutine add_state_to_space(ilut, called_from, nI_in)
        ...
        if (called_from == called_from_semistoch) then
            ...
            if (tTruncInitiator) then
                flags = ibset(flags, flag_initiator(1))
                flags = ibset(flags, flag_initiator(2))
            end if
    ...
    subroutine generate_optimised_core(called_from)
        ...
        do i = 1, this_proc_size
            ...
            if (tTruncInitiator) then
                call set_flag(SpawnedParts(:,i), flag_initiator(1))
                call set_flag(SpawnedParts(:,i), flag_initiator(2))
            end if
