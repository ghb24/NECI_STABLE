...
contains
    ...
    function get_csf_helement_local(nI, nJ, iLutI, iLutJ, nopen, nclosed, &
        ...
        if ((nopen(1) == nopen(2)) .and. (nup(1) == nup(2))) then
            dets2 = dets1
        else
    ...
    pure subroutine csf_get_yamas(nopen, sfinal, yama, ncsf_max)
        ...
        do npos = nopen, 2, -1
            do csf = 1, ncsf
                if (spin(csf, npos) < npos) then
                    spin(csf, npos - 1) = spin(csf, npos) + 1
                    yama(csf, npos) = 2
                    if (spin(csf, npos) /= 0) then
                        ncsf_next = ncsf_next + 1
                        if (ncsf_next > ncsf_max) exit
