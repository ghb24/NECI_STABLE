...
!        write(6,*) "Det is allowed by excitlevel: ",CrossIC
        ...
        !j = ex(1,2)
        if (CrossIC == 1) then
            if (SpinOrbSymLabel(ExCross(1, 1)) /= SpinOrbSymLabel(ExCross(2, 1))) then
                !symmetry forbidden
                tcross_conn = .false.
            else
                tcross_conn = .true.
            ...
            if (SymB /= SpinOrbSymLabel(ExCross(2, 2))) then
                !symmetry forbidden
                tcross_conn = .false.
            else
                tcross_conn = .true.
    ...
    subroutine find_invsym_permut(nINonOrder, nIOrder, tPermute)
        ...
        do i = 1, NEl
            ...
            do j = Nel, i + 1, -1
                ...
                if ((.not. tFoundOrb) .and. (nITemp(j) /= nIOrder(i))) then
                    cycle
                else
