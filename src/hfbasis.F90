...
!.. Now sum in the alpha and beta block of u matrix
        ...
        DO L = 1, NSBASIS
            ...
            IF (abs(DMAT(K, L, ISPN2)) > DmatEpsilon) TOT = TOT + (RHFMUL) * DMAT(K, L, ISPN2) * get_umat_el(ID1, ID3, ID2, ID4)
!                           WRITE(6,"(4I3)", advance='no') I,J,K,L
!                           WRITE(6,*) (RHFMUL)*
!     &                  DMAT(K,L,ISPN2)*GetUMatEl(NBasisMax,UMat,ALAT,
!     &                     NSBASIS*NSPINS,ISS,G1,ID1,ID3,ID2,ID4)
            IF (ISPN2 == ISPN .AND. (abs(DMAT(K, L, ISPN2)) > DmatEpsilon)) TOT = TOT - DMAT(K, L, ISPN2) * get_umat_el(ID1, ID3, ID4, ID2)
!                           IF(ISPN2.EQ.ISPN)
!     &                        WRITE(6,*)
!     &                  DMAT(K,L,ISPN2)*GetUMatEl(NBasisMax,UMat,ALat,
...
! Generate an initial determinant of specified spin.
    use SystemData, only: BasisFN, tMolpro, brr, g1, nbasis, lms, nel, tGiovannisBrokenInit, SymRestrict, tSymSet, nBasisMax, tHub, tGUGA
...
SUBROUTINE GLM_BROKEN_GENFDET(BRR, G1, NBASIS, LMS, NEL, FDET)
    use SystemData, only: BasisFN, tMolpro, IPARITY, nbasisMax, symmetry, symmax
...
!        ^minimum no of open orbitals
    if (ISYM%s /= 0 .and. minop == 0) minop = 2
...
!************************************************************************
    do while (.not. SymFound .and. nOpen <= maxop)
...
!****  CASE 2:  nOPEN = 1                                           *****
!************************************************************************
        elseif (nOpen == 1 .and. nOrb(ISYM%s + 1) > 0) then
...
! if true   condition pick next element (it happens only for alpha)
                IREAL = IREAL + 1
            END DO
            IF (I <= n_left(is) .and. IREAL /= 2 * norbOffset(ISYM%s + 1)) THEN
                NELCount = NELCount + 1
                FDET(NELCount) = BRR(IREAL)
                ioff(ISPN) = IREAL
            END IF
...
!           two alpha for triplet).
            jsym%s = -1
            Energy = 0.0d0
            do while (jsym%s < SYMMAX - 1)
                SymFoundTmp = .false.
                jsym%s = jsym%s + 1
                if (norb(jsym%s + 1) > 0) then
                    lsym = SYMPROD(jsym, ISYM)
                    countTotSym = 0
                    if (LMS /= 0 .and. ISYM%s == 0) countTotSym = 1
                    if (norb(lsym%s + 1) > countTotSym) then
                        FDET_tmp(1) = 2 * norbOffset(jsym%s + 1)     ! alpha electron
                        FDET_tmp(2) = 2 * norbOffset(lsym%s + 1) - 1   ! beta electron (for Ms = 0)
                        if (LMS /= 0) FDET_tmp(2) = 2 * (norbOffset(lsym%s + 1) + countTotSym)
...
! Fill up left over orbitals in canonical ordering
                    ...
                    DO IS = 1, 2
                        ...
                        DO WHILE (G1(BRR(IREAL))%Ms /= (-3 + 2 * ISPN))
                            IREAL = IREAL + 1
                        END DO
                        IF (((NELCount + 1) <= n_left(is)) .and. (BRR(IREAL) /= FDET_tmp(1) + 1 - IS)) THEN
                            if ((LMS == 0 .and. (BRR(IREAL) /= FDET_tmp(2) + 2 - IS)) &
                                .or. (LMS /= 0 .and. (BRR(IREAL) /= FDET_tmp(2) + 1 - IS))) then
                                NELCount = NELCount + 1
...
!                 call write_det (6, fDet_tmp, .true.)
                    call collapseDet(FDET_tmp)
!                ^ Collapse electrons to lowest orbitals within each irrep
!                 write(6,*) 'Energy =',get_helement(fDet_tmp,fDet_tmp,0)
                    if (real(get_helement(fDet_tmp, fDet_tmp, 0)) < Energy) then
...
SUBROUTINE collapseDet(FDET)
    use SystemData, only: G1, symmax, BasisFN, nbasisMax, nel
    Use Determinants, only: write_det
    use util_mod, only: int_fmt
    use sort_mod
    ...
    type(BasisFN) :: s
    integer, intent(inout) :: FDET(*)
    integer :: CounterOrb, CounterSym, CounterDet, CounterTot
    integer :: is, j
    ...
    do IS = 1, 2
        ...
        do while (G1(CounterOrb)%Sym%s == j)
        if ((G1(CounterOrb)%Sym%s == G1(FDet(CounterDet))%Sym%s) .and. (G1(CounterOrb)%Ms == G1(FDet(CounterDet))%Ms)) then
            if (CounterOrb < FDet(CounterDet)) FDet(CounterDet) = CounterOrb
...
SUBROUTINE StringGenerator(IS, ioff, OpnOrb, n_opn, String, StringSym)
    use SystemData, only: G1, BRR, nBasisMax, BasisFN, symmetry
    Use Determinants, only: write_det_len
    use sym_mod, only: getsym, writesym
    implicit none
    INTEGER Is, IREAL, ioff, NELString, I, OpnOrb, n_opn, String(*), iSpn
...
#ifdef CMPLX_
    call stop_all('CALCDEDCIJ2', 'HF not implemented for complex orbitals.')
#endif
