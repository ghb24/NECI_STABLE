...
def super_module(template, config, silent):
    """
...
def adj_arrays(template, config):
    ...
    # from any arrays which have them.
    split_ints = []
    last_end = 0
    new_template2 = ""
...
def interface_procs(template, silent):
    """
...
def process_file(fin, fout, silent=False, multifile=False):
    """
    This routine actually does the work!
    """
    config = read_config(fin, silent)

    # Read in the template
    template = fin.read()
    (template, super_mod) = super_module(template, config, silent)
    template = interface_procs(template, silent)

    # If we want to output the submodules into different files, then we
    # need to know what base filename we are using
    filelist = [fout.name]
    if multifile:
        dirname = os.path.dirname(fout.name)
        basename = os.path.basename(fout.name)
        root, ext = os.path.splitext(basename)

    # Write the output file
    for counter, s in enumerate(config):
    ...
    # We return the list of written files, for use in other processing utilities
    return filelist
