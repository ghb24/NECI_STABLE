...
START_ENVIRONMENTS = {'@{}'.format(env): '\\verbatimLaTeX{{\\begin{{{}}}}}'.format(env) for env in ENVIRONMENTS}
END_ENVIRONMENTS = {'@end{}'.format(env): '\\verbatimLaTeX{{\\end{{{}}}}}'.format(env) for env in ENVIRONMENTS}
...
def process_markdown_file(f):
    ...
    while line:
        ...
        if stripped in START_ENVIRONMENTS.keys():
            ...
            processed_file.append(color_regex.sub(r'\\textcolor{\1}{\2}', line))
