...
\definecolor{mgreen}{rgb}{0.1, 0.9, 0.1}

% % \renewcommand{\textcolor}[2]{}
% % \def\tmp\colortext
% \let\tmp\colortext
% \renewcommand{\colortext}[2]{\texttt{\tmp{#1}{#2}}}
% % \let\colortext\tmp
...
\newcommand\headitem[1]{\item[{\boldmath #1 \nopagebreak}] \textbf{#1} \hfill \\ \nopagebreak}
\newcommand\headit[1]{\item[\textnormal{\itshape #1 \nopagebreak}] \textbf{#1} \hfill \\ \nopagebreak}
% \newcommand\codeitem[1]{\item[\textnormal{\ttfamily #1 \nopagebreak}] {\tt #1} \hfill \\ \nopagebreak}
% TODO I think the problem is that textcolor goes around \item (!)
% \newcommand\codeitem[1]{\item \lstinline{system \nopagebreak}}
% \item \textcolor{red}{\lstinline{system} \nopagebreak}
\newcommand\codeitem[1]{\textbf{#1} \hfill \\ \nopagebreak}
...
\subsubsection{Overriding packages required for options}
    ...
    \begin{itemize}
        \item \codeitem{NECI\_FIND\_<package>}
        ...
        \item \codeitem{<package>\_LIBRARIES}
        ...
        \item \codeitem{<package>\_DEFINITIONS}
            Any additional compiler flags that are required to use the package.
        \item \codeitem{<package>\_INCLUDE\_PATH}
...
\subsection{Configuring builds (Makefile system)}
	...
	\begin{itemize}
		\item \codeitem{gfortran\_simple, ifort\_simple}
		...
		\item \codeitem{fkf\_ifort}
			...
			compile there.
		\item \codeitem{PC-ifort64-MPI-TARDIS, PC-ifort64-MPI-HYDRA}
...
\subsection{Git overview}
	...
	\item \codeitem{git push origin branchname}
		...
		of the work
	\item \codeitem{git checkout -b newbranchname --track origin/remotebranch}
...
\begin{itemize}
    \item \textcolor{red}{\codeitem{system}}
    % \textcolor{\listinline{red}{system}}
    % \item \textcolor{red}{\lstinline{system} \nopagebreak}
  ...
  \begin{itemize}
    ...
    \item \codeitem{FCIDUMP-name}
    Specify the name of the FCIDUMP file. Defaults to FCIDUMP.
    \item \codeitem{hubbard}
    Uses the Hubbard model Hamiltonian.
    \begin{itemize}
	\item \codeitem{k-space} In the momentum-space basis (beneficial for low $U/t$)
	\item \codeitem{real-space} In the real-space basis (beneficial for large $U/t$)
    \end{itemize}
    \item \codeitem{ueg}
    Uses the Hamiltonian of the uniform electron gas in a box.
  \end{itemize}
  \item \textcolor{mred}{\codeitem{endsys}}
  Terminates the SYSTEM block.
  \item \textcolor{mred}{\codeitem{electrons $n$, nel $n$}}
  Sets the number of electrons to $n$
  \item \codeitem{spin-restrict [$m$]}
  Sets the total $S_z$ quantum number to $\frac{m}{2}$. The argument $m$ is
  optional and defaults to 0.
  \item \codeitem{hphf $s$}
  ...
  \item \codeitem{sym $k_x$ $k_y$ $k_z$ $s$}
  ...
  \item \codeitem{complexOrbs\_realInts}
  ...
  \item \codeitem{complexWalkers-realInts}
  ...
  \item \codeitem{system-replicas $n$}
...
\begin{itemize}
  \item \textcolor{oblue}{\codeitem{nonUniformRandExcits}}
  ...
  \begin{itemize}
   \item \textcolor{oblue}{\codeitem{pchb}}
    ...
    \item \codeitem{4ind-weighted-unbound}
    Generates excitations using the same Cauchy-Schwary estimate as
    ...
    \item \codeitem{mol-guga-weighted}
...
\begin{itemize}
  \item \codeitem{GAS-SPEC}
    ...
    It is possible to use \emph{local} or \emph{cumulative}
      constraints on the particle number.
    Local constraints define the minimum and maximum particle number per
      GAS space.
    Cumulative constraints define cumulative minima and maxima
      of the cumulative particle number.
    The specification is first \texttt{LOCAL} or \texttt{CUMULATIVE}
      to define the kind of constraints
      followed by the number of GAS spaces $n_\text{GAS}$.
    The next items are $3 \times n_\text{GAS}$ numbers
      which are the number of spatial orbitals and (cumulative) minimum and maximum
      number of particles per GAS space
      $n_i, N_i^\text{min}, N_i^\text{max}$.
    Finally an integer array denotes for each spatial orbital to which GAS space it belongs.
    Instead of \texttt{1 1 1 1 1} one can write \texttt{5*1}.
    ...
    GAS-SPEC CUMULATIVE 2 +++
         ...
         6*1 6*2
    \end{lstlisting}
    In the given example the local and cumulative constraints are equivalent,
      but they are not always!

  \item \codeitem{GAS-CI}
    \emph{Optional keyword.}
    Specify the actual implementation for GAS.
    If it is ommitted, it will be deduced from \texttt{GAS-SPEC}.
    \begin{itemize}
      \item \codeitem{GENERAL-PCHB}
        This is the default and the fastest implementation, if sufficient memory is available.
        The double excitations work similar to the FCI precomputed heat bath excitation
          generator but automatically exclude GAS forbidden excitations.
        If one follows the keyword, by \texttt{SINGLES}, one can
          select the singles for which there are three possibilities:
        \begin{itemize}
          \item \codeitem{PC-UNIFORM}
            This is the default. It chooses GAS allowed electrons uniformly.
          \item \codeitem{DISCARDING-UNIFORM}
            It chooses electrons uniformly as in FCI and discards.
          \item \codeitem{ON-FLY-HEAT-BATH}
      ...
      \item \codeitem{GENERAL}
        Use heat bath on the fly general GAS,
          which is applicable to any GAS specification,
          but a bit slower than necessary for disconnected spaces.
      \item \codeitem{DISCONNECTED}
...
\end{itemize}
  ...
  \begin{itemize}
    \item \codeitem{lattice $type$ $l_x$ $l_y$ [$l_Z$]}
    ...
    \item \codeitem{twisted-bc $t_1$ [$t_2$]}
    ...
    $\frac{2\Pi}{y}$.
    \item \codeitem{open-bc [$direction$]}
    ...
    \item \codeitem{ueg-offset $k_x$ $k_y$ $k_z$}
  ...
  \begin{itemize}
    \item \codeitem{molecular-transcorr}
    ...
    \item \codeitem{ueg-transcorr $mode$}
    ...
    \item \codeitem{2-body-transcorr [$J$]}
    ...
    \item \codeitem{exclude-3-body-ex}
  ...
  \begin{itemize}
    \item \textcolor{mred}{\codeitem{calc}}
    Starts the CALC block
    \item \textcolor{mred}{\codeitem{endcalc}}
    Terminates the CALC block
    \item \textcolor{oblue}{\codeitem{time $t$}}
    Set the maximum time $t$ in minutes the calculation is allowed to
    run. After $t$ minutes, the calculation will end.
    \item \textcolor{oblue}{\codeitem{nmcyc $n$}}
    Set the maximum number of iterations the calculation is allowed to
    do. After $n$ iterations, the calculation will end.
    \item \codeitem{eq-cyc $n$}
    Set the number of iterations to perform after reaching variable shift
    mode. $n$ Iterations after entering the variable shift mode, the
    calculation will end. If both \code{eq-cyc} and \code{nmcyc} are given,
    the calculation ends when either of the iteration limits is reached.
    \item \codeitem{seed $s$}
    ...
    \item \codeitem{load-balance-blocks [OFF]}
  ...
  \begin{itemize}
    \item \textcolor{mred}{\codeitem{totalWalkers $n$}}
    ...
    \item \textcolor{oblue}{\codeitem{diagShift $S$}}
    ...
    \item \codeitem{targetGrowRate $grow$ $walks$}
  ...
  \begin{itemize}
    \item \textcolor{oblue}{\codeitem{allRealCoeff}}
    ...
    \item \textcolor{oblue}{\codeitem{realSpawnCutoff $x$}}
    ...
    \item \codeitem{energy-scaled-walkers [$mode$ $\alpha$ $\beta$]}
  ...
  \begin{itemize}
    \item \codeitem{\textcolor{mred}{tau $\tau$} [SEARCH]}
    ...
    calculation stable.
    \item \textcolor{oblue}{\codeitem{hist-tau-search [$c$ $nbins$ $bound$]}}
    ...
    \item \textcolor{oblue}{\codeitem{max-tau $\tau_\text{max}$}}
    Sets the maximal value of the time-step to $\tau_\text{max}$. Defaults to
    $1$.
    \item \codeitem{min-tau [$\tau_\text{min}$]}
    ...
    \item \codeitem{maxWalkerBloom $n$}
  ...
  \begin{itemize}
    \item \textcolor{oblue}{\codeitem{walkContGrow}}
    When reading in a wave function from a file, do not set the shift or enter
    variable shift mode.
    \item \textcolor{oblue}{\codeitem{defineDet $det$}}
    ...
    \item \codeitem{proje-changeRef [$frac$ $min$]}
  ...
  \begin{itemize}
    ...
    \item \codeitem{coherent-superInitiators [$mode$]}
    ...
    \item \codeitem{dynamic-superInitiators $n$}
    ...
    will be virtuals).
    \item \codeitem{ras-initiator ras1 ras2 ras3 ras4 ras5}
    ...
    \item \codeitem{optimised-initiator}
    ...
    \item \codeitem{optimised-initiator-cutoff-amp $x1$, $x2$, $x3$...}
    ...
    \item \codeitem{optimised-initiator-cutoff-num $n1$, $n2$, $n3$...}
    Perform the optimised initiator option, and in iteration $i$, choose
  ...
  \begin{itemize}
    \item \codeitem{auto-adpative-shift [$t$ $\alpha$ $c$]}
    ...
    \item \codeitem{linear-adaptive-shift [$\sigma$ $f_1$ $f_2$]}
    ...
    \item \codeitem{exp-adaptive-shift [$\alpha$]}
    ...
    \item \codeitem{core-adaptive-shift}
  ...
  \begin{itemize}
    \item \codeitem{multiple-initial-refs}
    ...
    \item \codeitem{orthogonalise-replicas}
    ...
    \item \codeitem{orthogonalise-replicas-symmetric}
    Use the symmetric L{\"o}wdin orthonaliser instead of Gram Schmidt for
    orthogonalising the replicas.
    \item \codeitem{replica-single-det-start}
    Starts each replica from a different excited determinant.
  \end{itemize}

  \subsubsection{Semi-stochastic options}
  \begin{itemize}
    \item \textcolor{oblue}{\codeitem{semi-stochastic}}
    Turn on the semi-stochastic adaptation.
    \item \textcolor{oblue}{\codeitem{pops-core $n$}}
    This option will use the $n$ most
    populated determinants to form the core space.
    \item \codeitem{doubles-core}
    Use the reference determinant and all single and double excitations
    from it to form the core space.
    \item \codeitem{cas-core cas1 cas2}
    Use a CAS space to form the core space. The parameter cas1 specifies
    the number of electrons in the cas space and cas2 specifies the
    number of virtual spin orbitals (the cas2 highest energy orbitals
    will be virtuals).
    \item \codeitem{ras-core ras1 ras2 ras3 ras4 ras5}
  ...
  \begin{itemize}
    \item \textcolor{oblue}{\codeitem{trial-wavefunction [$n$]}}
  ...
  \begin{itemize}
    ...
    \item \codeitem{memoryFacSpawn $x$}
  ...
  \begin{itemize}
    ...
    \item \codeitem{strict-inits-rdm}
    Require both sides of the inits-rdm to be initiators.
    \item \codeitem{no-lagrangian-rdms}
  ...
  \begin{itemize}
    \item \textcolor{mred}{\codeitem{methods}}
    Starts the METHODS block
    \item \textcolor{mred}{\codeitem{endmethods}}
    Terminates the METHODS block.
    \item \textcolor{mred}{\codeitem{method $mode$}}
  ...
  \begin{itemize}
    \item \codeitem{integral}
    Starts the INTEGRAL block.
    \item \codeitem{endint}
    Terminates the INTEGRAL block.
    \item \codeitem{freeze $n$ $m$}
  ...
  \begin{itemize}
    \item \codeitem{\devcolor{UNIT-TEST-PGEN}}
  ...
  \begin{itemize}
    \item \codeitem{kp-fciqmc}
    Starts the KP-FCIQMC block
    \item \codeitem{end-kp-fciqmc}
    Terminates the KP-FCIQMC block.
    \item \codeitem{num-krylov-vecs $N$}
    $N$ specifies the total number of Krylov vectors to sample.
    \item \codeitem{num-iters-between-vecs $N$}
    ...
    function.
    \item \codeitem{num-iters-between-vecs-vary $i_{12}$, $i_{23}$, $i_{34}$...}
    ...
    \item \codeitem{num-repeats-per-init-config $N$}
    ...
    the final results.
    \item \codeitem{averagemcexcits-hamil $N$}
    ...
    \item \codeitem{finite-temperature}
    ...
    \item \codeitem{num-init-configs $N$}
    ...
    spawning dynamic.
    \item \codeitem{fully-stochastic-hamil}
    ...
    \item \codeitem{init-correct-walker-pop}
    ...
    \item \codeitem{init-config-seeds seed1, seed2...}
    ...
    \item \codeitem{scale-population}
  ...
  \end{itemize}
    ...
    \begin{itemize}
      ...
      the shift.
      \item \codeitem{energy-benchmark $E$}
    ...
    \begin{itemize}
      ...
      output.
      \item \codeitem{fval-energy-hist}
    ...
    \begin{itemize}
      ...
      \item \codeitem{write-most-pop-core-end $n$}
    ...
    \begin{itemize}
      \item \codeitem{calcRdmOnfly $i$ $step$ $start$}
      ...
      both are produced.
      \item \codeitem{rdmLinSpace $start$ $n$ $step$}
    ...
    \item \codeitem{writeRdmsEvery $iter$}
