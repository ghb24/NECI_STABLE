...
\subsection{Overriding configuration options}
    ...
    self explanatory from reading the files in \cmake{cmake/compiler_flags}.
...
\subsubsection{Toolchain files (for example Archer)}
    ...
    set the \code{CMAKE_<lang>_COMPILER_ID} variable to \code{GNU} such that the compiler flags
    set in the \code{cmake/compiler_flags/GNU_*.cmake} files are used. Because this turns off
...
\subsubsection{Overriding packages required for options}
    ...
    \begin{description}
        ...
        \codeitem{<package>_INCLUDE_PATH}
