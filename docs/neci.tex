...
To generate the tags file, a program called \code{ctags} is needed. It is installed  by default in many Linux distributions, so most probably you will already find it on you machine.
If not, you can download and install it from here: \url{https://github.com/universal-ctags/ctags}.
Once you are sure \code{ctags} executable exists, go to the source directory of NECI and run the script \code{generate_tags.sh} which is available in the tools directory
\begin{lstlisting}[gobble=4]
	 cd neci/src
    ../tools/generate_tags.sh
 \end{lstlisting}
This will generate the necessary \code{tags} file in the current directory and code navigation should be come available for all source files in this directory.
