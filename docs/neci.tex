...
\begin{description}
	...
	\headitem{{\ttfamily use} statements}
        ...
        \headitem{Nested functions and subroutines}
                Within a function or subroutine, nested functions or
                subroutines can be declared, following a {\ttfamily contains}
                statement. The nested function will be within the scope of the
                parent function and can therefore access all variables which
                are in scope in the outer function. This can be used both to avoid code duplication and
                to avoid passing unnecessary arguments.

                Consider using nested subroutines/functions to
                encapsulate recurrent parts of a subroutine/function without
                exposing them to module scope.

                A nested subroutine/function cannot make use of the {\ttfamily
                contains} statement.
