...
\subsubsection{Special case: Archer}
	Archer uses a number of custom compiler wrappers. These wrappers
	defeat the auto-configuration built into the CMake system. As such
	manual specification of the compilers will be needed:
	\begin{lstlisting}[gobble=4]
		cmake -DCMAKE_Fortran_COMPILER=ftn -DCMAKE_CXX_COMPILER=g++ [path_to_neci]
	\end{lstlisting}
	The CMake system will detect that compilation is taking place on
	Archer and prompt the user to use these settings. If they are not
...
\section{Testing}
NECI comes with a set of tests in the test_suite directory. Each of these
...
rdm, excited_state, kpfciqmc and so on, with tests for each of these
