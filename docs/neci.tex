...
\begin{description}
  ...
  \begin{description}
   \codeitem{\textcolor{oblue}{pchb}}
	...
	Needs the \texttt{guga} keyword in the \texttt{System} block. 
    If it is used in conjunction with the \texttt{hist-tau-search} option, which 
    is recommended for GUGA calculations in general, it automatically sets 
    more reasonable defaults than for the usual \texttt{mol-guga-weighted} 
...
\end{description}
  ...
  \begin{description}
    ...
    \codeitem{bipartite-order [n]}
    Enables a bipartite ordering of bipartite lattice (chain, square for now)
    Has to be put \textbf{before!} the \texttt{lattice} keyword. 
    If an additional parameter \texttt{n}, which has to equal to the number of lattice sites, is given an 
    arbitrary orbital ordering can be given in the subsequent input line. 
    (It has to be \texttt{n} unique numbers from, 1 to $n$.) An example:
    \begin{lstlisting}[gobble=4]
	bipartite 5
	1 3 2 5 4
    \end{lstlisting}
  ...
  \end{description}
    ...
    \begin{description}
      ...
      \codeitem{local-spin}
      Activates the cumulative local spin, $\hat S_i^2$, measurement for spin-adapted GUGA calculations. 
      An output file \texttt{local\_spin\_stats} is created, which keeps track of the local spin measurement up to every spatial orbital for each iteration is stored. Similar to diagonal RDM entries, this option needs two replicas for an unbiased measurement. Since this is a diagonal property for the GUGA, there is no additional cost.
    ...
    \begin{description}
      ...
      \codeitem{print-core-hamil}
      Prints the semi-stochastic Hamiltonian and the semistochastic basis states to the output file \texttt{semistoch-hamil} and \texttt{semistoch-basis}. 
      Caution with the size of the semistochastic space!
      
      \codeitem{print-core-vec}
      Prints the semi-stochastic ``eigenvector'' to the output files \texttt{determ\_vecs} and \texttt{determ\_vecs\_av}.
      Caution with the size of the semistochastic space!
    ...
    Output the spin-free 2-RDMs to disk at the end of the calculation.
     ...
     \codeitem{full-core-rdms}
     In the `normal' RDM sampling only entries, e.g. for the 1-RDM, $\rho_{ij}$, which are actually connected by 
     some Hamiltonian matrix element. This has no negative effect on the energy calculation, but for properties, e.g. spin-spin correlation functions. 
     This option activates a full sampling of RDMs, at least in the semi-stochastic space. 
...
\section{Using the NECI pylib}

A python frontend is available in the \code{neci_guga} python library, which
is based on the \code{neci} build target (i.e. plain neci, without complex
...
\code{neci_guga.<build-specifier>.so}, which is installed in the
\code{python/} subdirectory of the neci build directory.

To use it, load the \code{python} subdirectory of the neci build directory
...
The \code{neci_guga} python module can then be loaded in calling python code
with \code{import neci_guga} and provides the following functionality:

\begin{description}
  \codeitem{neci\_guga.init\_guga(S)}
  Takes the desired total spin \code{S} and initializes the GUGA functionality
  of neci by reading in an existing FCIDUMP file. 

  \codeitem{neci\_guga.clear\_guga()}
  Clears all memory and deletes all objects initialized by \code{init_guga},
  returns 0 on success, 1 else.

  \codeitem{neci\_guga.diag\_matel(D)}
  Returns the diagonal matrix element of a CSF \code{D}, passed as an array
  of the size of the number of electrons.

  \codeitem{neci\_guga.run\_neci(perm)}
  Reads a neci input file \code{neci.inp} and an FCIDUMP file in the current
  directory, using an orbital permutation \code{perm} to re-order the orbitals
